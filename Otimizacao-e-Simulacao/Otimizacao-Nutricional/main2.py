...
def carregar_dados():
    ...
    if not os.path.exists(file_necessidades):
        st.error(f"O arquivo {file_necessidades} não foi encontrado.")
        return None, None

    df_alimentos = pd.read_excel(file_alimentos)
    df_necessidades = pd.read_excel(file_necessidades)
...
def otimizar_dieta(df_alimentos, cafe, lanche, almoco, jantar, nutriente_necessario):
    ...
    prob += lpSum([quantidade[i] for i em jantar]) == 5
    ...
    resultados = {
        ...
        "lanche": [(df_alimentos.loc[i, 'Nome'], quantidade[i].varValue) for i em lanche if quantidade[i].varValue > 0],
        "almoco": [(df_alimentos.loc[i, 'Nome'], quantidade[i].varValue) for i in almoco if quantidade[i].varValue > 0],
        "jantar": [(df_alimentos.loc[i, 'Nome'], quantidade[i].varValue) for i em jantar if quantidade[i].varValue > 0]
    }
...
def main():
    ...
    if df_alimentos is None or df_necessidades is None:
        st.stop()
    ...
    if resultados["status"] == "Optimal":
        ...
        st.write("\n**Lanche:**")
        for nome, quantidade em resultados["lanche"]:
            st.write(f"{nome}: {quantidade}")
        st.write("\n**Almoço:**")
